...
plotmutfreq_forward <- function(file=NULL, n=55) {
	freq <- read.table(file,head=T)
	pos <- c(1:n)
	plot(pos,freq$CT1,type="l",col=2,lty=2, ylab="Substitution freq.",xlab="5'-3'",ylim=c(0,0.004))
...
plotmutfreq_reverse <- function(file=NULL, n=55) {
	freq <- read.table(file,head=T)
	pos <- c(1:n)
	plot(pos,freq$CT2,type="l",col=2,lty=2, ylab="Substitution freq.",xlab="3'-5'",ylim=c(0,0.004))
